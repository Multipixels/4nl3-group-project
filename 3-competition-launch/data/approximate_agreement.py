...
# Classify negative categories as same "agreement"
...
for game in identical_games_1:
    for message in data[0][game]:
        if message["label"] in "345": messages_1.append(3)
        else: messages_1.append(message["label"])
    for message in data[1][game]:
        if message["label"] in "345": messages_2.append(3)
        else: messages_2.append(message["label"])

for game in identical_games_2:
    for message in data[2][game]:
        if message["label"] in "345": messages_3.append(3)
        else: messages_3.append(message["label"])
    for message in data[3][game]:
        if message["label"] in "345": messages_4.append(3)
        else: messages_4.append(message["label"])
